...
app.get("/calculaIMC/style.css", (req, res) => {
  res.sendFile(__dirname + "/src/input.css");
});
...
app.post("/calculaIMC", (req, res) => {
  ...
  if (
    transaccion.nombre != NaN &&
    transaccion.prestamo != NaN &&
    transaccion.interes != NaN &&
    transaccion.meses != NaN
  ) {
...
app.listen(port, () => {
  console.log(
    "El programa se está ejecutando por: http://localhost:8097/calculaIMC"
  );
});
